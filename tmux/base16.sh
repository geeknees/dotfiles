...
base02='#7D8000' # - Selection Background
...
# window colors
set-option -g window-style bg=$base04
set-window-option -g window-status-style fg=$base03,bg=$base00
# set-window-option -g window-status-format " #I #W"
set-window-option -g window-status-format '#[fg=white]#I #[fg=green]#W #[fg=blue]#(echo "#{pane_current_path}" | rev | cut -d'/' -f-2 | rev) '

# active window title colors
set-option -g window-active-style bg=$base01
set-window-option -g window-status-current-style fg=$base0C,bg=$base00
# set-window-option -g window-status-current-format " #[bold]#W"
set-window-option -g window-status-current-format '#[fg=yellow, bold]#W #[fg=red, bold]#(echo "#{pane_current_path}" | rev | cut -d'/' -f-2 | rev) '
...
tm_session_name="#[default,fg=$base0E,bg=$base00]#{?client_prefix,#[reverse],} #S "
set-option -g status-left "$tm_session_name"

# tm_host="#[fg=$base0E]#[fg=$base00,bg=$base0E] #h "
tm_date="#[fg=$base0D]#[fg=$base00,bg=$base0D] %Y/%m/%d(%a) %H:%M "
tm_ip="#[fg=$base0C]#[fg=$base00,bg=$base0C] #(ipconfig getifaddr en1 || ipconfig getifaddr en0)"
set-option -g status-right "$tm_host $tm_ip $tm_date "
